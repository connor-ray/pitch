     ...
     success: function (data) {
      // need to create another array or string with all coordinates of each object, then pass into generate map function, coordinates must be string separated by commas
       ...
       for (var i = 0; i < objects.length; i++){
         ...
         $("#locationList").append("<form class='' action='/proposals' method='post'><input type='hidden' name='item[location_title]' value='" + objectData[0] + "'><input type='hidden' name='item[address]' value='" + objectData[1] + "'><input type='hidden' name='item[distance]' value='" + objectData[2] + "'><input type='hidden' name='item[category]' value='" + objectData[3] + "'></form><input type='submit'>");
